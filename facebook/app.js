...
videoUrlInput.addEventListener('keydown', async (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'v') {
        try {
            ...
            if (text && text.trim()) {
                e.preventDefault();
                const start = videoUrlInput.selectionStart || 0;
                const end = videoUrlInput.selectionEnd || 0;
                const value = videoUrlInput.value;
                videoUrlInput.value = value.slice(0, start) + text.trim() + value.slice(end);
                const cursorPos = start + text.trim().length;
                videoUrlInput.setSelectionRange(cursorPos, cursorPos);
                hideMessages();
            }
        } catch (error) {
            // Browser may already handle native paste, so ignore permission issues.
        }
    }
});
